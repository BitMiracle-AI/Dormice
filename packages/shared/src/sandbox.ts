...
/**
 * The caller's identifier from their own system — a user id, a session id,
 * a CI job name — that acquire() is idempotent on: same external id, same
 * sandbox, whatever state it was in. sandboxId is the daemon's internal
 * identity; this is the external one, and it is the API's handle. Opaque to
 * the daemon; bounded so it can serve as an indexed column.
 */
