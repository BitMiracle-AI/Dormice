...
/**
 * API key management: mint, list, edit, revoke — the fleet's client
 * credentials, minted here at the one door (design record #20; nodes
 * accept only the fleet token). Pure table verbs. Admin-only:
 * buildGatewayApp registers this plugin behind requireAdminAuth (fleet
 * token or console session; a live key gets an honest 403), because a
 * credential must not manage the credential ledger it lives in.
 * Verification itself lives in db/api-keys.ts and is consulted by the
 * app's isCredential closure, not here.
 */
