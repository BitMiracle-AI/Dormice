...
/**
 * The fleet's front door, over the wire: bind a domain from the console
 ...
 * with the reason — never a silent no-op. Behind the admin gate (design
 * record #9): an automation key must not be able to rewrite the fleet's
 * Caddyfile.
 */
  ...
  app.post(
    ...
    async (request) => {
      if (!ingress) {
        throw httpError(
          400,
          'this gateway manages no reverse proxy — set DORMICE_INGRESS_FILE in its env (install.sh sets up Caddy and points it at /etc/caddy/Caddyfile), or configure your proxy directly',
        );
      }
