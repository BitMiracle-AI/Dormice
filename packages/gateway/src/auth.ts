...
function sessionCookieValid(
  ...
  // The jar exists where the app registered @fastify/cookie (app.ts does).
  // Typed structurally so this module needs no plugin type augmentation.
 ...
 * key, adjudicated by isCredential) and the web console's session
 ...
 *
 * isCredential judges bare tokens, so both faces (this Bearer header and
 * the E2B X-API-KEY hook) feed it the same canonical form — one closure,
 * one truth, two dialects. The 'Bearer ' prefix is public framing, not a
 * secret, so stripping it needs no constant time; the secret comparisons
 * live inside isCredential.
 */
...
/**
 * The admin gate for everything that configures the fleet — the apiKey
 * management verbs, the settings, templates, ingress and node verbs
 * (design record #9): only the env token (Bearer) or a console session
 * may pass. A key that is otherwise valid gets an honest 403 instead of a
 * silent 401 — key-manages-key would let one leaked credential mint itself
 * an unrevoked successor and revoke every legitimate peer, and a leaked
 * automation key must not be able to raise the very limits that contain
 * it or re-point the templates every node boots from; the refusal names
 * the rule. The console-setup door (routes/console.ts) rests on the same
 * doctrine: a machine credential must not escalate into managing the
 * fleet.
 *
  ...
  return async (request, reply) => {
    ...
    if (bare !== null && isLiveApiKey(bare)) {
      await reply.code(403).send({
        message:
          'API keys cannot manage API keys, settings, templates, domains or nodes — use DORMICE_API_TOKEN or the console',
      });
      return;
    }
    await reply.code(401).send({ message: 'missing or invalid API token' });
