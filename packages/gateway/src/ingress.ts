...
/**
 * The fleet's front door: a Caddy config file the gateway owns, rewritten
 * on setIngress and reloaded into the running Caddy. The file is the
 * single source of truth — no table column, no boot-time reconcile: an
 * operator can read it, Caddy runs it, and getIngress parses it back. TLS
 * is entirely Caddy's job (ACME issuance and renewal); this class only
 * decides what the file says. The one Caddy with a public face in a fleet
 * sits in front of the gateway (design record #24): the API domain and
 * the sandbox wildcard both land here, the nodes' own Caddys face the
 * intranet only.
 *
...
export interface IngressOptions {
  /** The Caddy config file the gateway owns (DORMICE_INGRESS_FILE). */
  filePath: string;
  /** Where the proxy forwards to: the gateway's own loopback port. */
  upstreamPort: number;
  /**
   ...
   * the gateway owns the whole file; an operator whose own Caddyfile imports
...
export class Ingress {
   ...
   * or the file is not ours — a foreign file's site addresses are not the
   * gateway's to report.
   */
  domains(): string[] {
    ...
    if (
      ...
      throw new UnmanagedIngressFileError(
        `${this.filePath} was not written by Dormice — refusing to overwrite it; ` +
          'move your configuration elsewhere, or point DORMICE_INGRESS_FILE at a file the gateway may own',
      );
    }
