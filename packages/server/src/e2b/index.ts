...
import { signedFileRoutes } from './signed-files';
 ...
 *
 * The third registration has no prefix: signed file URLs land on
 * `<daemon>/files` because the SDK's `new URL('/files', sandboxUrl)`
 * replaces the whole path — see signed-files.ts.
 */
export async function registerE2bCompat(
  ...
  await app.register(signedFileRoutes, deps);
}
