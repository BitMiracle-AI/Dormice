 ...
 * opens the door is adjudicated by isCredential (buildApp's closure: env
 * token or any active ledger API key), the same truth the native Bearer
 * face consults.
 */
export function isApiKey(
  ...
  return bare !== undefined && isCredential(bare);
}
