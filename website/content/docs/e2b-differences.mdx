...
description: What of the E2B surface is implemented — verified through the official SDK — and where Dormice deliberately diverges.
---
...
- **No template builds.** Templates are named images: build one with
  ...
  resolves it. The hosted product's build pipeline (`e2b template build`)
  is not implemented.
...
- **`metadata.userKey`** turns `Sandbox.create` into Dormice's idempotent
  acquire — see [Using the official E2B SDKs](/docs/e2b-sdks).
...
The daemon reports envd `0.6.1`, so the SDK gates these client-side
