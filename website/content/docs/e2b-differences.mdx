...
This page is the compatibility contract for applications written
against [E2B](/docs/core-concepts#what-is-e2b): which parts of the E2B
surface Dormice implements, where it deliberately behaves differently,
and what is honestly missing. If you are not migrating from E2B, you
can skip it.
...
| `uploadUrl()` / `downloadUrl()` | signed URLs: expiry enforced, a tampered signature is a 401; CORS-open, so a browser can POST/GET them directly — with a sandbox domain configured, E2B's `49983-<sandboxId>.<domain>/files` form works too |
...
| `getHost(port)` | port proxy with wake-on-traffic (configure a sandbox domain in the console) |
...
- **One domain, not one subdomain per sandbox.** The SDK addresses
  ...
  wildcard domain (the console's sandbox domain setting: one canonical
  domain that outbound URLs speak, plus optional inbound-only aliases).
